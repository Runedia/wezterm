...
impl TermWindow {
    pub async fn new_window(mux_window_id: MuxWindowId) -> anyhow::Result<()> {
        ...
        // Resolve the WebGPU state before borrowing `tw`, so that the
        // `.await` does not span an outstanding RefCell borrow
        // (clippy::await_holding_refcell_ref / 런타임 패닉 위험 방지).
